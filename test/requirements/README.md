# Requirements Tests

This directory is reserved for explicitly promoted product requirement tests.

Promoted requirement suites currently cover:

- the command surface under `test/requirements/commands/...`
- assistant chat usage under `test/requirements/assistant/...`
- direct tool behavior under `test/requirements/tools/...`
- proactive reminder behavior under `test/requirements/reminders/...`
- runtime model routing under `test/requirements/runtime/...`

Add tests here only when the behavior has been intentionally elevated from supporting coverage under `test/supporting/...`.
